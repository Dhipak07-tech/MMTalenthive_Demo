...
export function ResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setErrorMessage('Reset token is missing from the URL. Please verify your recovery email link.');
      return;
    }
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-slate-900 text-white p-16 flex-col justify-between relative overflow-hidden select-none">
        ...
        <div className="my-auto relative z-10 max-w-lg space-y-6">
          ...
          <p className="text-slate-400 text-sm leading-relaxed">
            Please configure your new login password. Standard policy requires at least 8 characters with multi-character complexity to protect organizational data.
          </p>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 relative">
        <div className="w-full max-w-md space-y-6">
          <div className="space-y-1 text-center lg:text-left">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Create a secure password for your user account.</p>
          </div>
            ...
            {!isSuccess ? (
              ...
              <div className="space-y-4 text-center">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  Your new login credentials have been saved. You can now use them to enter the workforce portal.
                </p>
              </div>
            )}
